...
$query1 = "SELECT * FROM tblofficials JOIN tblposition ON tblofficials.position=tblposition.id WHERE tblposition.position NOT IN ('SK Chairman','Secretary','Treasurer')
                AND `status`='Active' ORDER BY `order` ASC LIMIT 7";
...
$complainant = $resident['complainant'] ?? 'Unknown';
logActivity($conn, "PRINT", "BLOTTER_REPORT", trim($complainant), "Generated Blotter Report with Complainant: " . trim($complainant));
?>
<!DOCTYPE html>
<html lang="en">
