...
class DB
{
	...
	public function whereIn(string $key, array $values)
	{
		...
		if ($this->_where === "") {
			$this->_where = " WHERE {$key} IN ($joined_values) ";
		} else {
			$this->_where .= " AND {$key} IN ($joined_values) ";
		}
	...
	public function whereNotIn(string $key, array $values)
	{
		...
		if ($this->_where === "") {
			$this->_where = " WHERE {$key} NOT IN ($joined_values) ";
		} else {
			$this->_where .= " AND {$key} NOT IN ($joined_values) ";
		}
...
// Auto-Log Ended Events
// Check if column exists, if not add it
$check_col = $conn->query("SHOW COLUMNS FROM tblevents LIKE 'log_ended'");
if($check_col->num_rows == 0){
    $conn->query("ALTER TABLE tblevents ADD COLUMN log_ended TINYINT DEFAULT 0");
}

// Log logic (Only if someone is logged in to trigger it)
if(isset($_SESSION['id'])){
    // Find events that have ended but not yet logged
    $ended_events = $conn->query("SELECT * FROM tblevents WHERE end < NOW() AND log_ended = 0 AND status='Active'");
    if($ended_events){
        while($row = $ended_events->fetch_assoc()){
            $eid = $row['id'];
            $etitle = $conn->real_escape_string($row['title']);
            $uid = $_SESSION['id'];
            
            $log_action = "ENDED";
            $log_target = "Event";
            $log_details = $conn->real_escape_string("Event '$etitle' has ended.");
            
            $insert_log = "INSERT INTO activity_log (user_id, action, target_type, target_name, details) VALUES ('$uid', '$log_action', '$log_target', '$etitle', '$log_details')";
            if($conn->query($insert_log)){
                $conn->query("UPDATE tblevents SET log_ended = 1 WHERE id = '$eid'");
            }
        }
    }
}
