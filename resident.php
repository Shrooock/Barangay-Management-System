...
$residentList = (function () use ($db) {
	...
	if (isset($_GET["filter"])) {
		...
		switch ($filter) {
			case 'minor':
				$query->where("residents.age", "<=", 18);
				break;
			case 'adult':
				$query->where("residents.age", ">", 18);
				break;
			case 'pwd':
				$query->where("residents.is_pwd", 1);
				break;
			case 'voter':
				$query->where("residents.voterstatus", "Yes");
				break;
			case 'non_voter':
				$query->where("residents.voterstatus", "No");
				break;
			case 'senior':
				$query->where("residents.is_senior", 1); // Or age >= 60 depending on how you want to track it
				break;
            case '4ps':
                $query->where("residents.is_4ps", 1);
                break;
            case 'solo_parent':
                $query->where("residents.is_solo_parent", 1);
                break;
            case 'head_of_family':
                $query->where("residents.is_head_of_family", 1);
                break;
            case 'resident':
                $query->where("residents.resident_type", 1);
                break;
            case 'tenant':
                $query->where("residents.resident_type", 2);
                break;
		}
	}
...
<body>
    ...
    <div class="main-panel">
                  ...
                  <div class="card-head-row">
                    ...
                    <?php if (isAdmin()): ?>
                      <div class="card-tools">
                        <div class="d-inline-block mr-2" style="width: 200px;">
                            <select class="form-control form-control-sm" onchange="location.href='resident.php?filter=' + this.value">
                                <option value="">Filter By...</option>
                                <option value="" <?= !isset($_GET['filter']) ? 'selected' : '' ?>>All Residents</option>
                                <option value="minor" <?= isset($_GET['filter']) && $_GET['filter'] == 'minor' ? 'selected' : '' ?>>Minors (18 below)</option>
                                <option value="adult" <?= isset($_GET['filter']) && $_GET['filter'] == 'adult' ? 'selected' : '' ?>>Adults (19 above)</option>
                                <option value="senior" <?= isset($_GET['filter']) && $_GET['filter'] == 'senior' ? 'selected' : '' ?>>Senior Citizens</option>
                                <option value="pwd" <?= isset($_GET['filter']) && $_GET['filter'] == 'pwd' ? 'selected' : '' ?>>PWDs</option>
                                <option value="solo_parent" <?= isset($_GET['filter']) && $_GET['filter'] == 'solo_parent' ? 'selected' : '' ?>>Solo Parents</option>
                                <option value="head_of_family" <?= isset($_GET['filter']) && $_GET['filter'] == 'head_of_family' ? 'selected' : '' ?>>Head of the Family</option>
                                <option value="resident" <?= isset($_GET['filter']) && $_GET['filter'] == 'resident' ? 'selected' : '' ?>>Residents (Owner)</option>
                                <option value="tenant" <?= isset($_GET['filter']) && $_GET['filter'] == 'tenant' ? 'selected' : '' ?>>Tenants</option>
                                <option value="4ps" <?= isset($_GET['filter']) && $_GET['filter'] == '4ps' ? 'selected' : '' ?>>4Ps Beneficiaries</option>
                                <option value="voter" <?= isset($_GET['filter']) && $_GET['filter'] == 'voter' ? 'selected' : '' ?>>Voters</option>
                                <option value="non_voter" <?= isset($_GET['filter']) && $_GET['filter'] == 'non_voter' ? 'selected' : '' ?>>Non-Voters</option>
                            </select>
                        </div>
                        <a
                                  ...
                                  <div class="form-button-action">
                                    ...
                                    <?php if (isAdmin()): ?>
                                      ...
                                      <a
                                        type="button"
                                        data-toggle="tooltip"
                                        href="model/remove_resident.php?id=<?= $row[
                                        	"id"
                                        ] ?>"
                                        onclick="return confirm('Are you sure to archive this person?');" class="btn btn-link btn-danger"
                  ...
                  <div class="col-md-8">
                    ...
                    <div class="row">
                      <div class="col">
                        <div class="form-group">
                          <label>Age</label>
                          <input type="number" class="form-control" placeholder="Enter Age" min="1" name="age" id="age" required readonly>
                        </div>
                      </div>
                      <div class="col">
                        <div class="form-group">
      ...
      <div class="modal fade" id="import-modal" tabindex="-1" role="dialog" aria-labelledby="importModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="importModalLabel">Import Residents (CSV)</h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <form method="POST" action="model/import_residents.php" enctype="multipart/form-data">
              <div class="modal-body">
                <div class="form-group">
                ...
                <div class="alert alert-info py-2">
                  <small>
                    Ensure your CSV follows the export format (National ID, First Name, Middle Name, Last Name, etc.)
                  </small>
                </div>
              </div>
              <div class="modal-footer">
  ...
  <script>
    $(document).ready(function() {
      ...
      // Age calculation for the edit modal
